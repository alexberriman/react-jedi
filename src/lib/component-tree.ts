 ...
 */
export function buildComponentTree(spec: UISpecification | ComponentSpec): ComponentNode {
  // If it's a complete UI specification, use the root component
  const rootSpec: ComponentSpec =
    "root" in spec ? (spec as UISpecification).root : (spec as ComponentSpec);
 ...
 */
export function findNodesByType(root: ComponentNode, type: string): ComponentNode[] {
...
export function transformTree(
  ...
  const transformedChildren = transformedNode.children.map((child) =>
