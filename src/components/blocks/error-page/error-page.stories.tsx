...
import { 
  BookOpen, ShoppingCart, Newspaper, UtensilsCrossed, 
  Monitor, Palette, Building2, Star, Phone, Coffee,
  Home, Mail, Calendar, User, Heart
} from 'lucide-react'
...
const meta = {
  ...
  argTypes: {
    variant: {
      control: 'select',
      options: [
        'friendly-404', 'technical-error', 'maintenance', 'coming-soon', 'search-suggestions',
        'blog', 'ecommerce', 'magazine', 'restaurant', 'saas', 'portfolio', 'corporate'
      ],
    },
    siteType: {
      control: 'select',
...
const popularLinks = [
  { label: 'Homepage', href: '/', description: 'Return to the main page', icon: <Home className="h-4 w-4" /> },
  { label: 'Documentation', href: '/docs', description: 'Browse our guides', icon: <BookOpen className="h-4 w-4" /> },
  { label: 'Support', href: '/support', description: 'Get help from our team', icon: <Mail className="h-4 w-4" /> },
  { label: 'Blog', href: '/blog', description: 'Read our latest posts', icon: <Newspaper className="h-4 w-4" /> },
  { label: 'Contact', href: '/contact', description: 'Get in touch', icon: <Phone className="h-4 w-4" /> },
  { label: 'Status', href: '/status', description: 'Check system status', icon: <Monitor className="h-4 w-4" /> },
]

// Website-type specific popular links
const blogLinks = [
  { label: 'Latest Articles', href: '/articles', description: 'Read our newest posts', icon: <BookOpen className="h-4 w-4" /> },
  { label: 'Categories', href: '/categories', description: 'Browse by topic', icon: <Star className="h-4 w-4" /> },
  { label: 'About the Author', href: '/about', description: 'Learn more about me', icon: <User className="h-4 w-4" /> },
  { label: 'Subscribe', href: '/subscribe', description: 'Get updates via email', icon: <Mail className="h-4 w-4" /> },
]

const ecommerceLinks = [
  { label: 'Best Sellers', href: '/bestsellers', description: 'Our most popular items', icon: <Star className="h-4 w-4" /> },
  { label: 'New Arrivals', href: '/new', description: 'Latest products', icon: <ShoppingCart className="h-4 w-4" /> },
  { label: 'Sale Items', href: '/sale', description: 'Great deals and discounts', icon: <Heart className="h-4 w-4" /> },
  { label: 'Customer Service', href: '/support', description: 'Need help with your order?', icon: <Phone className="h-4 w-4" /> },
]

const restaurantLinks = [
  { label: 'Our Menu', href: '/menu', description: 'View our full menu', icon: <UtensilsCrossed className="h-4 w-4" /> },
  { label: 'Reservations', href: '/reservations', description: 'Book a table', icon: <Calendar className="h-4 w-4" /> },
  { label: 'Location & Hours', href: '/location', description: 'Find us and our hours', icon: <Phone className="h-4 w-4" /> },
  { label: 'Order Online', href: '/order', description: 'Get delivery or takeout', icon: <Coffee className="h-4 w-4" /> },
]

const portfolioLinks = [
  { label: 'Featured Work', href: '/portfolio', description: 'My best projects', icon: <Palette className="h-4 w-4" /> },
  { label: 'About Me', href: '/about', description: 'Learn more about my background', icon: <User className="h-4 w-4" /> },
  { label: 'Contact', href: '/contact', description: 'Let\'s work together', icon: <Mail className="h-4 w-4" /> },
  { label: 'Blog', href: '/blog', description: 'Thoughts and insights', icon: <BookOpen className="h-4 w-4" /> },
]
...
export const MagazineErrorPage: Story = {
  args: {
    siteType: 'magazine',
    popularLinks: [
      { label: 'Latest Stories', href: '/latest', description: 'Our newest articles', icon: <Newspaper className="h-4 w-4" /> },
      { label: 'Trending', href: '/trending', description: 'Most popular content', icon: <Star className="h-4 w-4" /> },
      { label: 'Subscribe', href: '/subscribe', description: 'Get our newsletter', icon: <Mail className="h-4 w-4" /> },
      { label: 'Archives', href: '/archives', description: 'Browse past issues', icon: <BookOpen className="h-4 w-4" /> },
    ],
    searchConfig: {
      enabled: true,
...
export const SaaSErrorPage: Story = {
  args: {
    siteType: 'saas',
    popularLinks: [
      { label: 'Dashboard', href: '/dashboard', description: 'Access your account', icon: <Monitor className="h-4 w-4" /> },
      { label: 'Documentation', href: '/docs', description: 'API guides and tutorials', icon: <BookOpen className="h-4 w-4" /> },
      { label: 'Support Center', href: '/support', description: 'Get help and answers', icon: <Mail className="h-4 w-4" /> },
      { label: 'System Status', href: '/status', description: 'Check service health', icon: <Star className="h-4 w-4" /> },
    ],
    searchConfig: {
      enabled: true,
...
export const CorporateErrorPage: Story = {
  args: {
    siteType: 'corporate',
    popularLinks: [
      { label: 'About Us', href: '/about', description: 'Learn about our company', icon: <Building2 className="h-4 w-4" /> },
      { label: 'Services', href: '/services', description: 'What we offer', icon: <Star className="h-4 w-4" /> },
      { label: 'Contact', href: '/contact', description: 'Get in touch', icon: <Phone className="h-4 w-4" /> },
      { label: 'Careers', href: '/careers', description: 'Join our team', icon: <User className="h-4 w-4" /> },
    ],
    showBreadcrumb: true,
    breadcrumbItems: [
...
export const Enhanced404WithSearch: Story = {
  args: {
    ...
    description: 'The page you\'re looking for seems to have taken a vacation. Let\'s help you find what you need.',
    popularLinks: popularLinks.slice(0, 4),
    searchConfig: {
      enabled: true,
    ...
    contactEmail: 'help@example.com',
    customActions: [
      { label: 'Report Issue', href: '/report', variant: 'outline' },
    ],
  },
}
